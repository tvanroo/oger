...
$teamsUrl = "https://go.microsoft.com/fwlink/p/?linkid=869428"
$teamsInstallerPath = Join-Path -Path $prepPath -ChildPath "Teams_windows_x64.msi"

Write-Host "Downloading the Teams installer..."
Invoke-WebRequest -Uri $teamsUrl -OutFile $teamsInstallerPath

Write-Host "Installing Teams in per-machine mode..."
Start-Process -FilePath $teamsInstallerPath -ArgumentList "ALLUSERS=1" -Wait

Write-Host "Completed the process to install Teams in per-machine mode." -ForegroundColor Green
